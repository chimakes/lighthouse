...
export function setupUI(water, ground, fireflies, camera) {
    ...
    gui.add(water.material.uniforms.uOpacity, 'value').min(0).max(1).step(0.01).name('Water Opacity')
    
    // gui.add(water.material.uniforms.uWavesAmplitude, 'value').min(0).max(1).step(0.01).name('Amplitude')
    gui.add(water.material.uniforms.uWavesFrequency, 'value').min(0.01).max(1).step(0.01).name('Wave Frequency')
    ...
    gui.add(water.material.uniforms.uWavesSpeed, 'value').min(0).max(2).step(0.001).name('Wave Speed')
